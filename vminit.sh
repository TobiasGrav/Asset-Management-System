...
JWTSECRET=$(sudo az keyvault secret show --name JWTSECRET --vault-name ams-secret-key-vault --query value -o tsv)
...
sudo docker run -d --name ams-backend -p 8080:8080 -e DATABASE_URL="51.13.61.6" -e DATABASE_USERNAME=root -e DATABASE_PASSWORD="${MYSQL_PASSWORD}" -e DATABASE_PASSWORD="${JWTSECRET}" --network ams-network amsprojectacr.azurecr.io/ams-backend:latest
